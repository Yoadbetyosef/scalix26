...
export async function getComponent(tenantId: string, id: string) {
  const { data } = await admin().from('product_components').select('*').eq('tenant_id', tenantId).eq('id', id).maybeSingle()
  return data ?? null
}
export async function createComponent(tenantId: string, productId: string, input: ComponentInput): Promise<{ ok: true; component: Record<string, unknown> } | { ok: false; error: string }> {
  ...
  if (input.costCents != null && !Number.isInteger(input.costCents)) return { ok: false, error: 'cost_must_be_integer_minor_units' }
