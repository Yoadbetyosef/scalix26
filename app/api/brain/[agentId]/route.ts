...
export async function GET(_req: NextRequest, { params }: { params: Promise<{ agentId: string }> }) {
  ...
  try {
    const [dnaRes, uRes, rRes, pRes, updRes] = await Promise.all([
      ...
      admin.from('brain_updates').select('kind, dna_strand, title, detail, delta, created_at').eq('tenant_id', tenantId).eq('ai_employee_id', agentId).order('created_at', { ascending: false }).limit(8),
    ])
    ...
    return NextResponse.json({ dna: [], understandings: [], recommendations: [], patterns: [], updates: [], patternsSummary: { total: 0, byCategory: {} }, learnedCount: 0, note: e instanceof Error ? e.message : 'not migrated' })
  }
}
