...
export async function POST(req: NextRequest) {
  ...
  // Get tenant's active AI employee to use their voice
  ...
  if (tenant) {
    ...
    if (employee?.voice && VOICE_MAP[employee.voice]) {
      voiceId = VOICE_MAP[employee.voice]
    }
  }
  ...
  const res = await fetch(`https://api.elevenlabs.io/v1/text-to-speech/${voiceId}`, {
    ...
    body: JSON.stringify({
      text: text.slice(0, 500), // limit length for speed
