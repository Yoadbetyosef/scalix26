...
// POST /api/studio/upload — multipart { file } → stores it in the tenant's folder and returns a stable
// public URL. Used by the document branding modal (drag & drop a logo). Mirrors /api/catalog/upload.
export async function POST(req: NextRequest) {
  ...
  if (!ext) return NextResponse.json({ error: `Unsupported image type (${file.type || 'unknown'}). Use PNG, JPG, WEBP or SVG.` }, { status: 400 })
