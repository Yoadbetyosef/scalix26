...
export async function GET(_req: Request, { params }: { params: Promise<{ token: string; attachmentId: string }> }) {
  ...
  if (!/^[0-9a-f-]{36}$/i.test(attachmentId)) return new NextResponse('Not found', { status: 404 })
  ...
  const { data } = await createAdminClient()
    ...
    .maybeSingle()
  if (!data || data.visibility !== 'public' || !publicAttachmentKind(data.mime_type as string)) {
