...
export default async function PartnerDashboard() {
  ...
  const healthColor = x.health.score >= 70 ? 'text-green-600' : x.health.score >= 40 ? 'text-amber-600' : 'text-red-500'
  ...
  return (
    <div className="space-y-6">
      ...
      <section className="rounded-2xl border border-accent/25 bg-gradient-to-br from-accent/5 to-transparent p-5 shadow-e1">
        <div className="mb-3 flex items-center gap-2"><Sparkles className="h-5 w-5 text-accent-strong" /><h2 className="text-lg font-semibold text-ink">Today&apos;s Focus</h2></div>
        {x.focus.length === 0 ? <p className="text-sm text-subtle">You&apos;re on top of it. Keep momentum — generate a demo or share your link.</p> : (
          <div className="grid gap-2.5 sm:grid-cols-3">
            {x.focus.map((f, i) => (
              <Link key={i} href={f.href} className="group flex flex-col justify-between rounded-xl border border-hairline bg-surface p-3.5 transition-shadow hover:shadow-e2">
                <div className="flex items-start gap-2.5"><span className="mt-0.5 text-accent-strong"><CoachIcon name={f.icon} className="h-5 w-5" /></span><span className="text-sm font-medium text-ink">{f.title}</span></div>
      ...
      {x.quickActions.length > 0 && (
        <div className="flex flex-wrap gap-2">
          {x.quickActions.map((q) => {
            ...
            return (
              <Link key={q.key} href={q.href} className={`inline-flex items-center gap-2 rounded-xl px-3.5 py-2 text-sm font-medium shadow-e1 transition-all hover:-translate-y-px hover:shadow-e2 ${q.primary ? 'bg-ink text-white' : 'border border-hairline bg-surface text-ink'}`}>
      ...
      <div className="grid grid-cols-2 gap-3 lg:grid-cols-5">
        ...
        <StatCard label="Lifetime Earned" value={money(stats.lifetime_earnings_cents)} />
      </div>

      {/* MONEY ON THE TABLE + BUSINESS HEALTH */}
      <div className="grid gap-4 lg:grid-cols-2">
        <div className="rounded-2xl border border-emerald-200 bg-gradient-to-br from-emerald-50 to-transparent p-5 shadow-e1">
          <div className="flex items-center gap-2 text-emerald-700"><DollarSign className="h-5 w-5" /><h2 className="text-sm font-semibold uppercase tracking-wide">Money left on the table</h2></div>
          <div className="mt-1 text-3xl font-bold text-ink">{money(x.moneyOnTable.monthly_cents)}<span className="text-base font-medium text-subtle">/mo waiting</span></div>
          {x.moneyOnTable.items.length === 0 ? (
            <p className="mt-2 text-sm text-subtle">Bring in prospects and trials — your unrealized recurring income shows up here.</p>
          ) : (
            <div className="mt-3 space-y-1.5">
              {x.moneyOnTable.items.map((m, i) => (
                <div key={i} className="flex items-center justify-between text-sm"><span className="text-subtle">{m.label}</span><span className="font-medium text-emerald-700">+{money(m.amount_cents)}/mo</span></div>
              ))}
              <Link href="/partner/pipeline" className="mt-1 inline-flex items-center gap-1 text-sm font-medium text-accent-strong hover:underline">Go close them <ArrowRight className="h-3.5 w-3.5" /></Link>
            </div>
          )}
        </div>

        <Panel title={<span className="inline-flex items-center gap-2"><Activity className="h-4 w-4 text-accent-strong" /> Business Health</span>}>
          <div className="flex items-center gap-4">
            <div className={`text-4xl font-bold ${healthColor}`}>{x.health.score}<span className="text-lg text-muted">/100</span></div>
            <div className="flex-1 space-y-1.5">
              {x.health.factors.map((f) => (
                <div key={f.label} className="flex items-center gap-2">
                  <span className="w-20 text-xs text-subtle">{f.label}</span>
                  <div className="h-1.5 flex-1 overflow-hidden rounded-full bg-sunken"><div className="h-full rounded-full bg-accent/70" style={{ width: `${Math.round((f.score / f.max) * 100)}%` }} /></div>
                </div>
              ))}
            </div>
          </div>
      ...
      {/* FORECAST */}
      <div>
        <div className="mb-2 flex items-center gap-2 text-sm font-semibold text-subtle"><TrendingUp className="h-4 w-4" /> 12-month forecast</div>
        <div className="grid grid-cols-2 gap-3 lg:grid-cols-4">
          ...
          <StatCard label="Projected Customers" value={x.forecast.customers} hint="At current pace" />
          <StatCard label="Projected Tier" value={x.forecast.level} />
        </div>
      </div>

      {/* PROGRESSION + WHY + TOP CHANNEL */}
      <div className="grid gap-4 lg:grid-cols-3">
        <Panel className="lg:col-span-2">
          <div className="flex flex-wrap items-center justify-between gap-3">
            ...
            <div className="text-right text-sm text-subtle"><div>Rank #{stats.global_rank ?? '—'} of {totalPartners ?? 1}</div><div className="text-xs text-muted">{stats.active_customers} active customers</div></div>
          </div>
          <div className="mt-3 h-2.5 w-full overflow-hidden rounded-full bg-sunken"><div className="h-full rounded-full bg-accent transition-all" style={{ width: `${lvl.progressPct}%` }} /></div>
          {lvl.nextLevel && benefits.length > 0 && (
            <div className="mt-3 rounded-xl bg-sunken/50 p-3">
              ...
              <div className="flex flex-wrap gap-x-4 gap-y-1">{benefits.map((b) => <span key={b} className="inline-flex items-center gap-1.5 text-sm text-ink"><CheckCircle2 className="h-3.5 w-3.5 text-accent-strong" /> {b}</span>)}</div>
            </div>
          )}
        </Panel>

        <Panel title={<span className="inline-flex items-center gap-2"><Radio className="h-4 w-4 text-accent-strong" /> Top Channel</span>}>
          {x.topChannel ? (
            <div>
              <div className="text-2xl font-semibold text-ink">{x.topChannel.label}</div>
              <div className="mt-0.5 text-sm text-subtle">{x.topChannel.customers} customer{x.topChannel.customers === 1 ? '' : 's'} acquired</div>
              <Link href="/partner/marketing" className="mt-3 inline-flex items-center gap-1 text-sm font-medium text-accent-strong hover:underline">Marketing OS <ArrowRight className="h-3.5 w-3.5" /></Link>
            </div>
          ) : <div className="py-4 text-sm text-muted">Launch a campaign or share a link to see your best-performing channel.</div>}
          <p className="mt-3 text-[11px] text-muted">Meta · Google · TikTok · LinkedIn sync in from the Marketing OS.</p>
        </Panel>
      </div>
      ...
      <Panel title="Your goals">
        <div className="grid gap-4 sm:grid-cols-3">
          {x.goals.map((g) => {
            ...
            return (
              <div key={g.key}>
                <div className="mb-1 flex items-center justify-between text-sm"><span className="text-subtle">{g.label}</span><span className="font-medium text-ink">{fmt(g.current)} / {fmt(g.target)}</span></div>
                <div className="h-2 overflow-hidden rounded-full bg-sunken"><div className="h-full rounded-full bg-accent" style={{ width: `${pct}%` }} /></div>
              </div>
            )
          })}
        </div>
      ...
      <div className="grid gap-6 lg:grid-cols-2">
        ...
        <div className="space-y-6">
          ...
          <Panel title="Recent activity">
            {x.activity.length === 0 ? <div className="py-6 text-center text-sm text-muted">Your activity will appear here as you build.</div> : (
            ...
            {x.ecosystem.length > 0 && (
              <div className="mt-3 border-t border-hairline pt-3">
                <div className="mb-1.5 text-xs font-semibold uppercase tracking-wide text-muted">Across the network</div>
                <div className="space-y-1.5">
                  {x.ecosystem.map((e, i) => (
                    <div key={i} className="flex items-center gap-2.5 text-sm text-subtle"><Flame className="h-3.5 w-3.5 text-orange-400" /><span className="flex-1">{e.label}</span><span className="text-xs text-muted">{relTime(e.at)}</span></div>
                  ))}
                </div>
              </div>
            )}
