...
import { Send, Bot, User, Zap, AlertCircle, Mic, MicOff, Phone, PhoneOff, MessageSquare } from 'lucide-react'
            ...
            <div className="text-center">
              ...
              {!listening && !speaking && !loading && callActive && (
                <button
                  ...
                  className="mt-4 flex items-center gap-2 px-4 py-2 bg-blue-50 text-blue-600 rounded-full text-sm hover:bg-blue-100 mx-auto"
                >
              ...
              <div className="flex flex-col items-center justify-center h-full text-gray-400 space-y-3">
                ...
                <div className="grid grid-cols-1 sm:grid-cols-2 gap-2 mt-2 w-full max-w-md">
                  {['I need to schedule an appointment', 'My AC is making a loud noise', 'How much does a furnace tune-up cost?', 'I have a gas leak emergency!'].map(s => (
                    <button key={s} onClick={() => setInput(s)} className="text-left px-3 py-2 text-xs text-gray-600 bg-gray-50 hover:bg-gray-100 rounded-lg border border-gray-100 transition-colors">{s}</button>
                  ))}
                </div>
              </div>
            )}
