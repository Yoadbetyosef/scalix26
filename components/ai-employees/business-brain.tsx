...
import { Volume2, Square, TrendingUp, Search, ShieldCheck, ArrowRight, Check, Lightbulb, Clock, HelpCircle, Wand2 } from 'lucide-react'
import { priorityOf, PRIORITY_META, cooStatement, estimatedImpact, dnaLine, openQuestions, studyLines, surprisedMe, COMING_NEXT, type Priority, type Sources } from '@/lib/brain/present'
...
interface BrainState { dna: Dna[]; understandings: Understanding[]; recommendations: Rec[]; patterns: Pattern[]; updates: Update[]; sources: Sources; dnaWeekDelta: Record<string, number>; lastLearned: string | null; learnedCount: number }
...
function timeAgo(iso: string | null): string {
  ...
  const h = Math.floor(mins / 60); if (h < 24) return `${h} hour${h === 1 ? '' : 's'} ago`
...
export function BusinessBrain({ agentId }: { agentId: string; agentName?: string }) {
  ...
  const execRecs = (s?.recommendations || []).map((r) => {
    const u = uById.get(r.understanding_id); const key = u?.understanding_key || ''
  ...
  async function speak() {
    ...
    try {
      ...
      await a.play()
    } catch { setSpeaking(false); toast.error("I couldn't brief you out loud right now.") }
  }

  return (
    <div className="space-y-7">
      {/* HERO — the COO morning briefing */}
      <div className="overflow-hidden rounded-3xl bg-gradient-to-br from-[#0d1230] via-[#141b40] to-[#241a48] p-6 text-white shadow-e2 sm:p-8">
        <div className="flex flex-col gap-5 sm:flex-row sm:items-start">
          <div className="relative flex-shrink-0">
            <span className={`absolute inset-0 rounded-full ${speaking ? 'animate-ping bg-white/20' : ''}`} />
            {/* eslint-disable-next-line @next/next/no-img-element */}
            <img src="/avatars/coo.png" alt="Your AI COO" className={`relative h-20 w-20 rounded-full object-cover ring-2 ${speaking ? 'ring-white/70' : 'ring-white/20'} transition-all`} />
          </div>
          <div className="min-w-0 flex-1">
            <p className="text-xl font-semibold sm:text-2xl">{greeting()}.</p>
            <p className="mt-0.5 text-sm text-white/75">
              {running ? "I'm studying your business right now…" : hasAnything ? `I've been studying your business — last time ${timeAgo(s?.lastLearned || null)}. Here's what I understand better today.` : "Let me study your business from the data you already have."}
            </p>
            {hasAnything && study.length > 0 && (
              <div className="mt-3 flex flex-wrap gap-x-4 gap-y-1.5">
                {study.map((x) => (
                  <span key={x.label} className="inline-flex items-center gap-1.5 text-xs text-white/80"><Check className="h-3.5 w-3.5 text-emerald-300" />{x.count} {x.label}</span>
                ))}
              </div>
            )}
          </div>
          <div className="flex flex-shrink-0 flex-col gap-2 sm:items-end">
            {hasAnything && (
              <button onClick={speak} className={`flex h-9 items-center gap-1.5 rounded-full px-3.5 text-xs font-medium text-white transition-colors ${speaking ? 'bg-white/25' : 'bg-white/15 hover:bg-white/25'}`}>
                {speaking ? <Square className="h-3.5 w-3.5" /> : <Volume2 className="h-4 w-4" />}{speaking ? 'Stop' : 'Hear my briefing'}
              </button>
            )}
      ...
      {!loading && !hasAnything && (
        <div className="rounded-2xl border border-hairline bg-white p-6 text-center">
          <p className="text-sm font-medium text-ink">I haven&apos;t studied your business yet.</p>
        ...
        <>
          ...
          <div className="rounded-2xl border border-hairline bg-white p-4">
            ...
            <div className="mt-3 grid gap-3 sm:grid-cols-2">
              <div>
                <p className="mb-1 text-[11px] font-medium uppercase tracking-wide text-subtle">Learning from</p>
                <div className="flex flex-wrap gap-1.5">
                  {study.map((x) => <span key={x.label} className="rounded-md bg-emerald-50 px-2 py-0.5 text-[11px] font-medium text-emerald-700">{x.label}</span>)}
                </div>
              </div>
              <div>
                <p className="mb-1 text-[11px] font-medium uppercase tracking-wide text-subtle">Coming next — each one gives me another sense</p>
                <div className="flex flex-wrap gap-1.5">
                  {COMING_NEXT.map((x) => <span key={x} className="rounded-md bg-gray-100 px-2 py-0.5 text-[11px] text-gray-500">{x}</span>)}
                </div>
              </div>
            </div>
          </div>
          ...
          {/* WHAT CHANGED TODAY */}
          <Section title="What changed today" icon={TrendingUp}>
            {(s?.updates || []).length === 0 ? (
              ...
              <div className="space-y-2">
                {s!.updates.slice(0, 5).map((u, i) => { const Icon = updIcon(u.kind); return (
                  <div key={i} className="flex items-start gap-2.5 rounded-xl border border-hairline bg-white p-3">
                    ...
                    <div className="min-w-0"><p className="text-sm font-medium text-ink">{u.title}{u.delta ? <span className="text-emerald-600"> +{u.delta}%</span> : null}</p>{u.detail && <p className="text-xs text-muted">{u.detail}</p>}</div>
                  </div>
                )})}
              </div>
            )}
          ...
          {/* WHAT I'VE LEARNED */}
          {learned.length > 0 && (
            <Section title="What I understand about your business" icon={Lightbulb}>
              <div className="space-y-2.5">
                {learned.map((u) => (
                  <div key={u.id} className="rounded-xl border border-hairline bg-white p-4">
                    <p className="text-[15px] font-medium text-ink">{cooStatement(u.understanding_key, u.statement)}</p>
                    <div className="mt-2 flex flex-wrap items-center gap-2 text-[11px]">
                      <span className="rounded-md bg-accent/10 px-2 py-0.5 font-medium text-accent-strong">{DNA_LABEL[u.dna_strand]} DNA</span>
                      <span className={`rounded-md px-2 py-0.5 font-medium ${u.business_confidence >= 65 ? 'bg-emerald-50 text-emerald-700' : 'bg-amber-50 text-amber-700'}`}>Business Confidence {u.business_confidence}%</span>
                      <span className="text-subtle">{u.evidence_summary}</span>
                    </div>
                  </div>
                ))}
              </div>
          ...
          {/* WHAT SURPRISED ME */}
          {surprised.length > 0 && (
            <Section title="What surprised me" icon={Search}>
              <div className="space-y-2">
                {surprised.map((t, i) => <div key={i} className="rounded-xl border border-amber-200 bg-amber-50/60 p-4 text-sm text-ink">{t}</div>)}
              </div>
            </Section>
          )}
          ...
          {needsEvidence.length > 0 && (
            <Section title="What still needs more evidence" icon={HelpCircle}>
              <div className="space-y-2">
                {needsEvidence.map((u) => (
                  <div key={u.id} className="rounded-xl border border-hairline bg-white p-3.5">
                    <p className="text-sm text-ink">{cooStatement(u.understanding_key, u.statement)}</p>
                    <p className="mt-1 text-[11px] text-subtle">Only {u.business_confidence}% confident so far · {u.evidence_summary} — I&apos;ll keep watching before I recommend anything.</p>
                  </div>
                ))}
              </div>
            </Section>
          ...
          {execRecs.length > 0 && (
            <Section title="Here's what I'd focus on">
              <div className="space-y-4">
                {PRIORITY_ORDER.map((prio) => {
                  const items = execRecs.filter((r) => r.priority === prio); if (!items.length) return null
                  const meta = PRIORITY_META[prio]
                  return (
                    <div key={prio}>
                      <div className="mb-1.5 flex items-center gap-1.5"><span className={`h-1.5 w-1.5 rounded-full ${meta.dot}`} /><span className={`rounded px-1.5 py-0.5 text-[11px] font-semibold ${meta.tone}`}>{meta.label}</span></div>
                      <div className="space-y-3">
                        {items.map((r) => (
                          <div key={r.id} className="rounded-2xl border border-hairline bg-white p-4 shadow-e1">
                            ...
                            <div className="mt-2.5 rounded-lg bg-sunken p-2.5"><p className="text-[11px] font-medium uppercase tracking-wide text-subtle">Estimated impact</p><p className="text-xs text-ink">{r.impact}</p></div>
              ...
              <div className="mt-3 flex items-start gap-2 rounded-xl border border-accent/20 bg-accent/5 p-3 text-xs text-ink">
                <Wand2 className="mt-0.5 h-4 w-4 flex-shrink-0 text-accent-strong" />
                <span>When I become confident enough, I&apos;ll go further on my own — suggesting improvements to your sales script, objection handling, and how leads get qualified. I&apos;ll always show you first.</span>
              </div>
            </Section>
          )}
            ...
            <Section title="Timeline" icon={Clock}>
              <div className="rounded-xl border border-hairline bg-white p-4">
                <ol className="space-y-3">
                  {s!.updates.slice(0, 8).map((u, i) => (
                    <li key={i} className="flex gap-3">
                      <span className="w-14 flex-shrink-0 pt-0.5 text-[11px] tabular-nums text-subtle">{new Date(u.created_at).toLocaleTimeString([], { hour: '2-digit', minute: '2-digit' })}</span>
                      <span className="mt-1 h-1.5 w-1.5 flex-shrink-0 rounded-full bg-accent" />
                      <span className="min-w-0 text-sm text-ink">{u.title}{u.delta ? <span className="text-emerald-600"> (+{u.delta}%)</span> : null}{u.detail ? <span className="text-muted"> — {u.detail}</span> : null}</span>
                    </li>
                  ))}
                </ol>
              </div>
          ...
          {/* BUSINESS DNA — alive */}
          <Section title="Your Business DNA">
            <div className="grid gap-2.5 sm:grid-cols-2">
              {DNA_ORDER.map((strand) => {
                const d = (s?.dna || []).find((x) => x.dna_strand === strand); const strength = d?.strength || 0
                const wk = s?.dnaWeekDelta?.[strand] || 0
                return (
                  <div key={strand} className="rounded-xl border border-hairline bg-white p-3.5">
                    <div className="flex items-center justify-between">
                      ...
                      <span className="flex items-center gap-1.5 text-xs tabular-nums text-subtle">{wk > 0 && <span className="text-emerald-600">↑ +{wk} this week</span>}{strength}%</span>
                    </div>
                    <div className="mt-2 h-1.5 w-full overflow-hidden rounded-full bg-sunken"><div className="h-full rounded-full bg-gradient-to-r from-accent to-[#A855F7] transition-[width] duration-700" style={{ width: `${Math.max(3, strength)}%` }} /></div>
          ...
          {questions.length > 0 && (
            <Section title="I'm still investigating…" icon={HelpCircle}>
              <div className="rounded-xl border border-hairline bg-white p-4">
                <ul className="space-y-1.5">
                  {questions.map((q, i) => <li key={i} className="flex items-start gap-2 text-sm text-ink"><ArrowRight className="mt-0.5 h-3.5 w-3.5 flex-shrink-0 text-subtle" />{q}<span className="text-subtle"> — I&apos;ll keep watching before I make a call.</span></li>)}
                </ul>
              </div>
            </Section>
          )}
