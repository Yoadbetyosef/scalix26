...
export function CalendarConnect({ agentId }: { agentId: string }) {
  ...
  useEffect(() => {
    ...
    if (p.get('calendar_connected')) toast.success('Google Calendar connected!')
    if (p.get('calendar_error')) toast.error('Could not connect Google Calendar. Please try again.')
    load()
  }, [])
  ...
  return (
    <div className="mt-4 rounded-xl border border-gray-200 p-4">
      <div className="flex items-center gap-2">
        ...
        <span className="font-semibold text-gray-800 text-sm">Google Calendar</span>
        {status?.connected && (
      ...
      {status === null ? (
        ...
        <div className="mt-2">
          <p className="text-xs text-gray-500 mb-3">Connect a Google Calendar to automatically add booked appointments as calendar events. Optional — booking works either way.</p>
