  ...
  if (done) {
    return (
      <div className="text-center py-4">
        ...
        <h2 className="text-lg font-bold text-ink">Thanks!</h2>
        <p className="text-subtle text-sm mt-1">We&apos;ll text you right away.</p>
      </div>
    )
  }
  ...
  return (
    <form onSubmit={submit} className="space-y-3">
      ...
      <textarea
        ...
        className="w-full border border-hairline-strong rounded-lg p-3 text-sm resize-none focus:outline-none focus:ring-2 focus:ring-[#5B6CF0]/40"
      />
      <Button type="submit" loading={submitting} className="w-full h-12 text-base">
        Get a callback
      </Button>
      {error && <p className="text-sm text-red-600 text-center">{error}</p>}
      <p className="text-xs text-muted text-center">We&apos;ll send you a text — standard rates may apply.</p>
    </form>
  )
}
