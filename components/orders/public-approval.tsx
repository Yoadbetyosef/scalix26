...
export function PublicApprovalForm({ token, approvalType, requestKind = 'approval' }: { token: string; approvalType: 'factory' | 'customer'; requestKind?: 'approval' | 'quote' }) {
  const [decision, setDecision] = useState<Decision | null>(null)
  const [comment, setComment] = useState('')
  const [estDate, setEstDate] = useState('')
  // A quotation: the factory says what it would cost. "Approve" here means "we can make it, at this
  // price"; the number is the answer and is required.
  const quote = requestKind === 'quote'
  ...
  const submit = async () => {
    ...
    const quotedCostCents = quote && cost.trim() !== '' ? Math.round((parseFloat(cost) || 0) * 100) : null
    if (quote && decision === 'approved' && !quotedCostCents) { setErr('Please enter your quoted cost.'); return }
    setBusy(true); setErr(null)
    try {
      const r = await fetch(`/api/approval/${token}`, { method: 'POST', headers: { 'Content-Type': 'application/json' }, body: JSON.stringify({ decision, comment: comment.trim() || null, estimatedCompletionDate: estDate || null, quotedCostCents }) })
  ...
  return (
    <div>
      ...
      <div style={{ display: 'flex', gap: 8, flexWrap: 'wrap', marginBottom: 12 }}>
        <button onClick={() => setDecision('approved')} style={{ ...btn(decision === 'approved' ? '#059669' : '#d1fae5'), color: decision === 'approved' ? '#fff' : '#065f46' }}>{quote ? 'Send quote' : 'Approve'}</button>
        <button onClick={() => setDecision('changes_requested')} style={{ ...btn(decision === 'changes_requested' ? '#d97706' : '#fef3c7'), color: decision === 'changes_requested' ? '#fff' : '#92400e' }}>{quote ? 'Need more information' : 'Request Changes'}</button>
        <button onClick={() => setDecision('rejected')} style={{ ...btn(decision === 'rejected' ? '#dc2626' : '#fee2e2'), color: decision === 'rejected' ? '#fff' : '#991b1b' }}>{quote ? 'Cannot quote' : 'Reject'}</button>
      </div>
      {decision && (
        <div style={{ marginBottom: 12 }}>
          {quote && decision === 'approved' && (
            <label style={{ fontSize: 12, color: '#6b7280', display: 'block', marginBottom: 8 }}>Your quoted cost<input inputMode="decimal" value={cost} onChange={(e) => setCost(e.target.value)} placeholder="0.00" style={input} /></label>
          )}
