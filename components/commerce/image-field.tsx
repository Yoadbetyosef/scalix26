...
// upload-from-computer (native <label>-wrapped file input so the picker reliably opens, incl. Safari;
// multipart → the given endpoint → catalog-images bucket) AND paste-URL — both save the SAME image_url (no
// duplicate field/media system). Immediate preview, replace, remove.
    ...
    <div className="space-y-2">
      <div className="flex items-center gap-3">
        ...
        <div className="flex flex-wrap items-center gap-2">
          <label className={cn(buttonVariants({ variant: 'outline', size: 'sm' }), 'cursor-pointer', busy && 'pointer-events-none opacity-60')}>
            <Upload className="h-4 w-4" /> {busy ? 'Uploading…' : value ? 'Replace' : 'Upload from computer'}
            <input type="file" accept="image/png,image/jpeg,image/webp,image/gif" disabled={busy} className="hidden" onChange={(e) => upload(e.target.files?.[0])} />
          </label>
      ...
      <div className="flex items-center gap-2">
        <div className="relative flex-1">
          ...
          <Input value={url} onChange={(e) => setUrl(e.target.value)} onKeyDown={(e) => { if (e.key === 'Enter') { e.preventDefault(); if (url.trim()) { onChange(url.trim()); setUrl('') } } }} placeholder="…or paste an image URL" maxLength={2000} className="pl-9" />
        </div>
