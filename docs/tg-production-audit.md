...
- **Production is deployed from the feature branch**, not `main` (`scalix26-git-fix-tg-jewellers-2` is aliased to `app.scalix26.com`). `main` is behind by round 2 and this work; the database is ahead of `main`. Merging to main without deploying the branch first is safe; deploying main as-is would regress round 2.
