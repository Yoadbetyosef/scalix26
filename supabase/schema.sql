...
-- Drip Campaigns — automated SMS follow-ups to open leads
...
CREATE POLICY "Tenant drip access" ON drip_campaigns FOR ALL USING (tenant_id = get_tenant_id());
CREATE INDEX idx_drip_due ON drip_campaigns(status, next_send_at);
CREATE INDEX idx_drip_phone ON drip_campaigns(tenant_id, contact_phone);
CREATE INDEX idx_drip_lead ON drip_campaigns(lead_id);
